...
foreach var in `vars' {
    ...
    tw ///
        ...
        saving("$FIGUREDIR/pirf_iv_`var'_usa.gph", replace)

    graph export "$FIGUREDIR/pirf_iv_`var'_usa.pdf", fontface($grfont) replace
    local ivar = `ivar'+1
}
restore

graph use "$FIGUREDIR/pirf_iv_approval_usa.gph",         name(g1, replace)
graph use "$FIGUREDIR/pirf_iv_GOVCRISIS_binary_usa.gph", name(g2, replace)
graph use "$FIGUREDIR/pirf_iv_STRIKE_binary_usa.gph",    name(g3, replace)
graph use "$FIGUREDIR/pirf_iv_DEMONSTR_binary_usa.gph",  name(g4, replace)

grc1leg2  g1 g2 g3 g4, ///
	cols(4) ///
    loff ///
	ysize(3) xsize(12) 
graph export "$FIGURECOM/PolEffects_Robustness_dropUSA.pdf", replace


******************************************************************************
* Robustness: Exclude Germany
******************************************************************************
preserve
local vars      approval GOVCRISIS_binary STRIKE_binary DEMONSTR_binary
local varsnames `" "Approval" "Government crises" "General strikes" "Demonstrations" "'
...
foreach var in `vars' {
    ...
    tw ///
        ...
        plotregion(color(white)) graphregion(color(white)) ///
        legend(off) ///
        name("ivD`ivar'", replace) ///
        saving("$FIGUREDIR/pirf_iv_`var'_deu.gph", replace)

    graph export "$FIGUREDIR/pirf_iv_`var'_deu.pdf", fontface($grfont) replace
    local ivar = `ivar'+1
}
restore

cap graph close _all
graph use "$FIGUREDIR/pirf_iv_approval_deu.gph",         name(g1, replace)
graph use "$FIGUREDIR/pirf_iv_GOVCRISIS_binary_deu.gph", name(g2, replace)
graph use "$FIGUREDIR/pirf_iv_STRIKE_binary_deu.gph",    name(g3, replace)
graph use "$FIGUREDIR/pirf_iv_DEMONSTR_binary_deu.gph",  name(g4, replace)

grc1leg2  g1 g2 g3 g4, ///
	cols(4) ///
    loff ///
	ysize(3) xsize(12) 
graph export "$FIGURECOM/PolEffects_Robustness_dropDEU.pdf", replace
