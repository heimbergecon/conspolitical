...
local varsnames_cum `" "Approval" "'
local varsnames `" "Approval" "Government crises" "General strikes" "Demonstrations" "'

local labels_cum `" "in %-points" "'
local labels `" "in %-points" "change in Pr in %-points" "change in Pr in %-points" "change in Pr in %-points" "'
...
cap erase "$FIGUREDIR/pirf_het3_approval_cum.gph"
...
foreach var in `vars' {
    ...
    forvalues i = 0/`horizon' {
	   ...
	   qui ivreg2 F`i'.`var' ///
            (`shock' shock_x = `Z' Z_x) ///
            `cntrls' `het_ctrls' i.year i.country_id if recession == 0, dkraay(1)
	...
	* Create significance markers sitting on each IRF line
    ...
    twoway ///
        ...
        saving("$FIGUREDIR/pirf_het3_`var'_UpperRegime.gph", replace)
    restore
    local ivar = `ivar' + 1
}
...
foreach var in `vars' {
    ...
    forvalues i = 0/`horizon' {
	   ...
	   qui ivreg2 F`i'.`var' ///
            (`shock' shock_x = `Z' Z_x) ///
            `cntrls' `het_ctrls' i.year i.country_id if recession == 1, dkraay(1)
	...
	* Create significance markers sitting on each IRF line
    ...
    twoway ///
        ...
        legend(order(3 "0% spending share" 6 "50% spending share" 9 "100% spending share") size(7) region(lcolor(none))) ///
        name("het3_`var'", replace) ///
        saving("$FIGUREDIR/pirf_het3_`var'_LowerRegime.gph", replace)
    restore
    local ivar = `ivar' + 1
}
...
// Upper Regime
graph use "$FIGUREDIR/pirf_het3_approval_UpperRegime.gph",         name(g1, replace)
graph use "$FIGUREDIR/pirf_het3_GOVCRISIS_binary_UpperRegime.gph", name(g2, replace)
graph use "$FIGUREDIR/pirf_het3_STRIKE_binary_UpperRegime.gph",    name(g3, replace)
graph use "$FIGUREDIR/pirf_het3_DEMONSTR_binary_UpperRegime.gph",  name(g4, replace)

grc1leg2 g1 g2 g3 g4, cols(4) ysize(3.5) xsize(12) 
graph export "$FIGURECOM/iv_political_spendshare_0_50_100_4panel_UpperRegime.pdf", replace

// Lower Regime
graph use "$FIGUREDIR/pirf_het3_approval_LowerRegime.gph",         name(g1, replace)
graph use "$FIGUREDIR/pirf_het3_GOVCRISIS_binary_LowerRegime.gph", name(g2, replace)
graph use "$FIGUREDIR/pirf_het3_STRIKE_binary_LowerRegime.gph",    name(g3, replace)
graph use "$FIGUREDIR/pirf_het3_DEMONSTR_binary_LowerRegime.gph",  name(g4, replace)

grc1leg2 g1 g2 g3 g4, cols(4) ysize(3.5) xsize(12) 
graph export "$FIGURECOM/iv_political_spendshare_0_50_100_4panel_LowerRegime.pdf", replace
