...
// Level specification
graph use "$FIGUREDIR/pirf_ivonly_approval.gph",          name(g1, replace)
graph use "$FIGUREDIR/pirf_ivonly_GOVCRISIS_binary.gph",         name(g2, replace)
graph use "$FIGUREDIR/pirf_ivonly_STRIKE_binary.gph",    name(g3, replace)
graph use "$FIGUREDIR/pirf_ivonly_DEMONSTR_binary.gph",     name(g4, replace)
set scheme s1color

grc1leg2 g1 g2 g3 g4, cols(4) ///
    loff ///
	ysize(3) xsize(12) 
graph export "$FIGURECOM/PolEffects_Headline.pdf", replace
